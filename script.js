...
function renderIndustryOptions(list){
  const sel = document.querySelector('#industrySelect'); if (!sel) return;
  list.forEach(i => { const opt = document.createElement('option'); opt.value = i.id; opt.textContent = i.name; sel.appendChild(opt); });
}
function renderIndustryGrid(list, filterId){
  const grid = document.querySelector('#industryGrid'); if (!grid) return;
  grid.innerHTML = '';
  const filtered = filterId ? list.filter(i => i.id === filterId) : list;
  filtered.forEach(i => {
    const card = document.createElement('article'); card.className = 'card panel fade-in';
    ...
    card.innerHTML = `<h3 class="title">${i.name}</h3><p class="muted">${i.summary}</p>
...
function onSelectIndustry(){ const id = document.querySelector('#industrySelect').value; renderIndustryGrid(window.__INDUSTRIES__, id || undefined); }
function switchTab(id){
  const tabs = document.querySelectorAll('[data-tab]'); const btns = document.querySelectorAll('.tab-btn');
  tabs.forEach(t => t.hidden = t.id !== id); btns.forEach(b => b.setAttribute('aria-selected', b.dataset.target === id ? 'true' : 'false'));
  if (id === 'tab-industries' && !window.__INDUSTRIES__) loadIndustries();
}
function setupSplash(){
  const s = document.getElementById('splash'); if (!s) return;
  const seen = sessionStorage.getItem('splashSeen'); if (seen === '1') { s.setAttribute('hidden','true'); return; }
  const SKIP = () => { s.classList.add('fade-out'); setTimeout(()=>{ s.setAttribute('hidden','true'); sessionStorage.setItem('splashSeen','1'); }, 800); };
  const skipBtn = document.getElementById('skipSplash'); skipBtn && skipBtn.addEventListener('click', SKIP);
